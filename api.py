...
def handle(refer_wav_path, prompt_text_path, prompt_language, text, text_language):
    f_open = open(prompt_text_path, 'r+', encoding='utf-8')  # 打开文本文件
    prompt_text = f_open.read()
    if (
...
async def tts_endpoint(request: Request):
    json_post_raw = await request.json()
    poet_id = json_post_raw.get("poet_id")
    if not poet_id:
        # 处理 poet_id 不存在的情况
        return {"error": "poet_id not found in request body"}
    gpt_path = f"./GPT_weights/{poet_id}.ckpt"
    sovits_path = f"./SoVITS_weights/{poet_id}.pth"
    print(f"gpt_path: {gpt_path}; sovits_path: {sovits_path}")
    # 假设这两个函数是同步的，如果是异步的，则需要添加await
    change_sovits_weights(sovits_path)
    change_gpt_weights(gpt_path)
    print("改变模型成功")
    refer_wav_path = f"./refer_wav_path/{poet_id}.wav"
    prompt_text_path = f"./prompt_text/{poet_id}.txt"
    print(f"refer_wav_path: {refer_wav_path}; prompt_text_path: {prompt_text_path}")
    prompt_language = "zh"
    text_language = "zh"
    text = json_post_raw.get("text")  # 移除多余的逗号
    # 调用handle函数并返回其结果
    return handle(refer_wav_path, prompt_text_path, prompt_language, text, text_language)
